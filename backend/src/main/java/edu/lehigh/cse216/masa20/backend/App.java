...
public class App {
	...
	if (cors_enabled.equals("TRUE")) {
		...
		return "";
            ...
            response.type("application/json");
	    ArrayList<Database.RowData> data = db.selectAllPosts();
	    if (data == null) {
		response.status(500);
        ...
        Spark.get("/messages/:id", (request, response) -> {
            ...
            response.type("application/json");
            Database.RowData data = db.selectOnePost(idx);
            if (data == null) {
		response.status(500);
	...
	Spark.put("/messages/:id", (request, response) -> {
	    ...
	    SimpleRequest req = gson.fromJson(request.body(), SimpleRequest.class);
	    response.type("application/json");
	    Database.RowData result = db.updateOnePost(idx, req.mTitle, req.mMessage);
	    if (result == null) {
		response.status(500);
